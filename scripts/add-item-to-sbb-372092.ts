/**
 * Adds 1x Dior "Backstage Glassy Glow Stick Multi-Use Highlighter Balm —
 * Glazed Pink" ($62) to existing order SBB-372092 (Ghadir Jaber). The order
 * already carried the note "add dior glassy glow stick multi use stick balm
 * glazed pink for 62$".
 *
 * Item is already sourced for $49 (marked sourced = true, cost_usd = 49).
 *
 * Mirrors app/api/admin/orders/[id]/items/route.ts for the totals recompute.
 *
 * Run:  npx ts-node scripts/add-item-to-sbb-372092.ts
 */
...
const SOURCED = true;
const VENDOR: string | null = null; // vendor not specified
const COST_USD = 49;
...
async function main(): Promise<void> {
  ...
  if (!prod.length) {
    console.error("Dior Glassy Glow Stick not found in products — run add-dior-glassy-glow-stick.ts first");
    process.exit(1);
  }
  const p = prod[0];
  ...
  await sql`
    ...
    values (
      ...
      ${p.price_usd}, ${p.price_gbp}, ${1}, ${SOURCED}, ${VENDOR}, ${COST_USD}
    )
  `;
  console.log(`  + ${p.brand} — ${p.name} ($${p.price_usd})  [sourced, cost $${COST_USD}]`);
