/**
 * New order for Farah Photography: 1x Sephora Collection All That Matte(r)s
 * Eye Palette at a special promo price of $29 (list price $38). Payment: cod,
 * confirmed — per standing instruction ([[lara_order_entry_defaults]]).
 * Reuses an existing customer with the same phone if there is one.
 *
 * Run:  npx tsx scripts/add-order-farah-photography-sephora-palette.ts
 */
...
const PRODUCT_ID = "b167cbfe-6869-4211-ae79-e4bd75d23bf5"; // Sephora Collection All That Matte(r)s Eye Palette

const CUSTOMER = {
  full_name: "Farah Photography",
  phone: "71317196",
  address: "Beirut"
};

const round2 = (n: number): number => Math.round(n * 100) / 100;
const SPECIAL_PRICE_USD = 29;
...
async function main(): Promise<void> {
  ...
  const priceUsd = SPECIAL_PRICE_USD;
  const priceGbp = SPECIAL_PRICE_GBP;
  ...
  console.log(`OK  ${order.order_number} — ${CUSTOMER.full_name} — ${p.brand} ${p.name} — $${priceUsd} — cod / confirmed (special promo price)`);
}
