/**
 * Adds Benefit Cosmetics Benetint - Rose-tinted Lip & Cheek Stain 6ml
 * (shade: Raspberry) to order SBB-536569.
 *
 * Run:  npx ts-node scripts/add-item-sbb-536569.ts
 */
...
async function main(): Promise<void> {
  ...
  const orderRows = (await sql`SELECT id FROM orders WHERE order_number = 'SBB-536569' LIMIT 1`) as Array<{ id: string }>;
  if (!orderRows.length) {
    console.error("Order SBB-536569 not found");
    process.exit(1);
  }
  ...
  const item = {
    ...
    name: "Benetint - Rose-tinted Lip & Cheek Stain 6ml — Colour: Raspberry",
    product_url: "https://seasonsbyb.co.uk/product/21eb2079-33bc-43e7-8c66-09a8e9b3c75e",
    image_url: "/benefit-benetint-raspberry.avif",
    price_gbp: 25.76,
    price_usd: 40,
    quantity: 1
  };
