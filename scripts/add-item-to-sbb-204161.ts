/**
 * Adds one line item to existing order SBB-204161 (Tia Abboud, Whish link,
 * already paid + ready to deliver) and refreshes the stored invoice:
 *   1x Charlotte Tilbury Airbrush Flawless Setting Spray 100ml
 *      (catalogue product 2244d9b8-d553-4dbe-896a-ef78f6f16e3e, catalogue price)
 *
 ...
 * order_items. Then regenerates the invoice PDF with lib/invoice.ts (it uses
 * the order's amount_paid_usd to show any balance due) and stores it in
 * invoice_pdf. It does NOT email or WhatsApp anyone and leaves
 * invoice_sent_at (the original send time) untouched.
 *
 * Run:  npx ts-node scripts/add-item-to-sbb-204161.ts
 */
...
const PRODUCT_ID = "2244d9b8-d553-4dbe-896a-ef78f6f16e3e";
...
async function main(): Promise<void> {
  ...
  const dupe = (await sql`
    select 1 from order_items where order_id = ${order.id} and product_url = ${p.product_url} limit 1
  `) as Array<unknown>;
  if (dupe.length) {
    console.error("This product is already on the order — aborting to avoid a duplicate line.");
    process.exit(1);
  }
